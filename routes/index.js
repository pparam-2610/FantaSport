...
router.get("/stages", async (req, res) => {
  //All Stages (Isme se select karke create league karega banda
  let data;
  let today = new Date();
  today.setDate(today.getDate() - 15);
  let after = new Date(today);
  after.setDate(after.getDate() + 35);
  ...
  for (let j = 0; j < stageData.length; j++) {
    ...
    await axios(config3)
      .then((resp) => {
        ...
        if (stageData[j].league_id != 3)
          stageData[j].name = resp.data.data.name;
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  res.render("home", { stageData: stageData, userData: req.user });
});
...
router.get("/upcoming", async(req, res) => {
  ...
  res.render("upcoming", { data: data, userData: req.user });
});

...
class leagues {
  static async getLeagueMatches(leagueDetails) {
    ...
    let dbLeagueData = await League.findOne({
      leagueCode: leagueDetails.leagueCode,
    });
    ...
    let config = {
      method: "get",
      url:
        "https://cricket.sportmonks.com/api/v2.0/fixtures?filter[stage_id]=" +
        stageId +
        "&sort=starting_at&api_token=" +
        process.env.API_KEY +
  ...
  static async viewPlayers(fixtureDetails) {
    ...
    let config1 = {
      ...
      url:
        ...
        fixtureData.localteam_id +
        "/squad/" +
        fixtureData.season_id +
        "?api_token=" +
        process.env.API_KEY,
      headers: {},
    };
  ...
  static async createTeam(teamData) {
    ...
    const leagueDetails = await League.findOne({
...
class privateLeagues {
  ...
  static async getPrivateLeagues(userDetails) {
    let privateLeagues = await League.find({
  ...
  static async joinPrivateLeague(leagueDetails) {
    const updateLeague = await League.findOne({
      leagueCode: leagueDetails.leagueCode,
      private: true,
    });
